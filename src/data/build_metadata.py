...
def build_unified_metadata(mass_train=None, mass_test=None,
    ...
    # Includes laterality (LEFT/RIGHT) alongside view — without it, a patient
    # with the same abnormality_id on both breasts would collide.
    metadata["image_id"] = (
        metadata["patient_id"].astype(str) + "_" +
        metadata["finding_type"] + "_" +
        metadata["abnormality_id"].astype(str) + "_" +
        metadata["laterality"].astype(str) + "_" +
